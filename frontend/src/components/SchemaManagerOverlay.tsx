/** Schema 管理全螢幕 Overlay（manager 以上可使用） */
...
function inferDataType(sampleValue: string, columnName: string): SchemaColumn['dataType'] {
  ...
  if (datePattern.test(v) || timeKeywords.some((k) => col.includes(k))) return 'time'
  const numVal = v.replace(/,/g, '')
  const numKeywords = ['金額', '數量', '營收', '銷售', 'amount', 'quantity', 'sales', 'count', 'price', 'value', 'profit', '毛利', '成本']
  if (/^-?\d+(\.\d+)?$/.test(numVal) || numKeywords.some((k) => col.includes(k))) return 'num'
  return 'str'
}
...
function parseColumnsFromSchema(sj: Record<string, unknown>): SchemaColumn[] {
  ...
  return Object.entries(cols).map(([colName, meta]) => {
    const rawSample = meta?.sample ?? meta?.sample_data
...
function buildColumnsToSchema(cols: SchemaColumn[]): Record<string, { type: string; attr: string; aliases: string[]; sample?: string }> {
  ...
  for (const c of cols) {
    ...
    const base = { type: c.dataType, attr: c.attr, aliases }
...
function normalizeIndicatorRowParts(raw: string[], wasComparePeriod: boolean): [string, string] {
  ...
  if (wasComparePeriod) return [t[0] ?? '', '']
  return [t[0] ?? '', t[1] ?? '']
}
...
function columnSelectLabel(c: SchemaColumn): string {
  ...
  const alias = (c.aliases || '').split(',').map((a) => a.trim()).filter(Boolean)[0]
...
function deriveSchemaFromCsv(rows: string[][]): SchemaColumn[] {
  ...
  return headers.map((col, i) => {
    ...
    return { columnName, dataType, attr: inferAttr(dataType), sampleData: sample, aliases: rawHeader }
  })
}
...
function ResizeHandle() {
  return (
    <Separator className="flex w-1 shrink-0 cursor-col-resize items-center justify-center bg-transparent outline-none ring-0 transition-colors hover:bg-gray-200/60 focus:outline-none focus:ring-0 focus-visible:outline-none focus-visible:ring-0">
...
export default function SchemaManagerOverlay({
  agentId,
  agentName,
  onClose,
  onSchemaChanged,
  ...
  const updateSchemaColumn = useCallback(
    (index: number, field: keyof SchemaColumn, value: string | SchemaColumn['dataType'] | SchemaColumn['attr']) => {
      setSchemaColumns((prev) => prev.map((col, i) => (i === index ? { ...col, [field]: value } : col)))
    },
    []
  )
  ...
  const handleCreate = useCallback(async () => {
    if (!schemaName.trim()) { setToast('請填寫名稱'); return }
    if (schemaColumns.length === 0) { setToast('請至少新增一個欄位'); return }
    setSaving(true)
    try {
  ...
  }, [schemaName, schemaDesc, schemaColumns, buildSchemaJson, loadSchemas, onSchemaChanged, agentId])

  const handleUpdate = useCallback(async () => {
    if (!selectedSchemaId || selectedSchemaId === NEW_SCHEMA_ID) return
    if (!schemaName.trim()) { setToast('請填寫名稱'); return }
    if (schemaColumns.length === 0) { setToast('請至少保留一個欄位'); return }
    setSaving(true)
    try {
  ...
  }, [selectedSchemaId, schemaName, schemaDesc, schemaColumns, buildSchemaJson, loadSchemas, onSchemaChanged])

  const handleSave = useCallback(async () => {
    if (selectedSchemaId === NEW_SCHEMA_ID) await handleCreate()
    else await handleUpdate()
  }, [selectedSchemaId, handleCreate, handleUpdate])

  const handleConfirmDeleteSchema = useCallback(async () => {
    if (!selectedSchemaId || selectedSchemaId === NEW_SCHEMA_ID) { setDeleteSchemaConfirmOpen(false); return }
    setDeleting(true)
    try {
  ...
  }, [selectedSchemaId, loadSchemas, onSchemaChanged])
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-white" role="dialog" aria-modal="true" aria-label="Schema 管理">
      {/* Overlay Header */}
      <div className="flex shrink-0 items-center justify-between border-b border-gray-200 bg-gray-700 px-6 py-4">
        <div>
          <h2 className="text-lg font-semibold text-white">Schema 管理</h2>
          {agentName && <p className="text-sm text-gray-300">{agentName}</p>}
        </div>
        <button
          type="button"
          onClick={onClose}
          className="rounded-lg p-2 text-white/70 transition-colors hover:bg-white/10 hover:text-white"
          aria-label="關閉 Schema 管理"
        >
          <X className="h-5 w-5" />
        </button>
      </div>
          ...
          >
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden p-4">
              <div className="mb-3 flex shrink-0 flex-wrap items-end justify-between gap-2 border-b border-gray-200 pb-0">
                <div className="flex gap-1" role="tablist" aria-label="Schema 與 Indicator 編輯">
                  <button
                    type="button"
                    role="tab"
                    aria-selected={editorTab === 'schema'}
                    aria-controls="smo-panel-schema"
                    tabIndex={editorTab === 'schema' ? 0 : -1}
                    onClick={() => setEditorTab('schema')}
                    className={`relative -mb-px rounded-t-lg px-4 py-2.5 text-sm font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${
                      editorTab === 'schema'
                  ...
                  <button
                    ...
                    aria-controls="smo-panel-indicator"
                        ...
                        {indicatorRows.length === 0 ? (
                          ...
                          <div className="overflow-auto rounded-lg border border-gray-200">
                            <table className="min-w-full border-collapse text-sm">
                              <thead className="sticky top-0 z-10 bg-gray-100">
                                <tr>
                                  <th className="border-b border-gray-200 px-3 py-2 text-left font-medium text-gray-700">指標 key</th>
                                  <th className="border-b border-gray-200 px-3 py-2 text-left font-medium text-gray-700">顯示名稱</th>
                                  <th className="border-b border-gray-200 px-3 py-2 text-left font-medium text-gray-700 min-w-[22rem] w-[min(100%,40rem)]">欄位（value_components）</th>
                                  <th className="border-b border-gray-200 px-3 py-2 text-center font-medium text-gray-700">百分比</th>
                                  <th className="w-12 border-b border-gray-200 px-2 py-2 text-center font-medium text-gray-700">刪除</th>
                                </tr>
                              </thead>
                              <tbody>
                                        ...
                                        <div className="flex w-full min-w-[20rem] flex-nowrap items-center gap-x-2">
                                          ...
                                          >
                                            ...
                                            {schemaFieldOptions.map((col, j) => {
                                              ...
                                              return <option key={`${v}-${j}`} value={v}>{columnSelectLabel(col)}</option>
                                            })}
                                            {(row.valueComponents[0] ?? '').trim() &&
                                              !schemaFieldOptions.some((c) => c.columnName.trim() === (row.valueComponents[0] ?? '').trim()) && (
                                                <option value={(row.valueComponents[0] ?? '').trim()}>
                                                  {(row.valueComponents[0] ?? '').trim()}（未列於 Schema）
                                                </option>
                                              )}
                                          </select>
                                          <span className="shrink-0 select-none text-sm text-gray-400" aria-hidden>/</span>
                                          <select
                                          ...
                                          >
                                            ...
                                            {schemaFieldOptions.map((col, j) => {
                                              const v = col.columnName.trim()
                                              return <option key={`d-${v}-${j}`} value={v}>{columnSelectLabel(col)}</option>
                                            })}
                                            {(row.valueComponents[1] ?? '').trim() &&
                                              !schemaFieldOptions.some((c) => c.columnName.trim() === (row.valueComponents[1] ?? '').trim()) && (
      ...
      <ConfirmModal
        ...
        variant="danger"
        onConfirm={() => { if (!deleting) void handleConfirmDeleteSchema() }}
        onCancel={() => { if (!deleting) setDeleteSchemaConfirmOpen(false) }}
      />

      {toast && (
        <div
          className="fixed bottom-8 left-1/2 z-[60] -translate-x-1/2 max-w-[90vw] rounded-lg bg-red-600 px-4 py-2 text-lg text-white shadow-lg"
          role="alert"
        >
          {toast}
        </div>
